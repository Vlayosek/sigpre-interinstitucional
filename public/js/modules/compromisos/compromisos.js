...
$("#gabinete_id_exportar_monitor").on("change", function() {
    app.onChangeInstitucion();
});

$("#gabinete_id_corresponsable_exportar_monitor").on("change", function() {
    app.onChangeInstitucionCorresponsable();
});
$("#provincia_id_exportar_monitor").on("change", function() {
    app.onChangeCantones();
});
//Para el filtro parroquias FC ago23
$("#canton_id_exportar_monitor").on("change", function() {
    app.onChangeParroquias();
});
...
function datatableCargarAntecedentes() {
    ...
    $("#dtmenuAntecedentes").dataTable({
        ...
        destroy: true,
        serverSide: true,
        ajax: "/compromisos/getDatatableAntecedentesServerSide/" + app.formCrear.id,
        //  stateSave:true,
        responsive: true,
        processing: true,
        lengthMenu: [
...
function datatableCargarHistorico() {
    ...
    $("#dtmenuHistorico").dataTable({
        ...
        destroy: true,
        // serverSide: true,
        ajax: "/compromisos/getDatatableHistoricoServerSide/" + app.formCrear.id,
        //   stateSave:true,
        responsive: true,
        processing: true,
        lengthMenu: [
...
function datatableCargarObjetivos() {
    ...
    $("#dtmenuObjetivos").dataTable({
        ...
        destroy: true,
        serverSide: true,
        ajax: "/compromisos/getDatatableObjetivosServerSide/" + app.formCrear.id,
        //   stateSave:true,
        responsive: true,
        processing: true,
        lengthMenu: [
        ...
        columns: arregloDatosObjetivos,
    });
    recargarBusquedas("OBJETIVOS");
}
...
function datatableCargarPeriodos() {
    if (
        ...
        editor = $("#dtmenuPeriodos").dataTable({
            ...
            destroy: true,
            serverSide: true,
            ajax: "/compromisos/getDatatablePeriodosServerSide/" +
                app.formCronograma.objetivo_id,
            stateSave: true,
            responsive: true,
            processing: true,
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        var cumplimiento_acumulado =
                            row.cumplimiento_acumulado != 0 && row.meta_acumulada != null ?
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        var html =
                            ((cumplido / row.suma_meta_acumulada) * 100).toFixed(2) + " %";
                        /*  if (isNaN(html))
                                      return 0;*/
                        return html;
                    },
...
//PESTAÑA BUSQUEDA
let tipoActual_busquedas = null;

function datatableCompromisosBusquedas(tipo = null) {
    if (tipo != null) tipoActual_busquedas = tipo;
    ...
    let monitor_busqueda =
        $("#monitor_busqueda").val() == "" || $("#monitor_busqueda").val() == null ?
        0 :
        $("#monitor_busqueda").val();

    $("#dtmenu_busquedas").dataTable({
        dom: "lBfrtip",
        destroy: true,
        serverSide: true,
        ajax: "/compromisos/getDatatableBusquedasServerSide/" +
...
$("#estado_porcentaje_id").on("change", function() {
    let tipo = $("#estado_porcentaje_id option:selected").text();
    if (
        tipo == "CERRADO" ||
        tipo == "CUMPLIDO" ||
        tipo == "EN PLANIFICACIÓN" ||
        tipo == "STAND BY"
    )
        app.formCrear.estado_id = "1";
    else app.formCrear.estado_id = "3";
});
...
function recargarBusquedas(tipo) {
    if (app.busquedas) {
        $(".cerrarmodal").click();
        if (tipo == "OBJETIVOS") $("#busquedasObjetivos").click();
        if (tipo == "MENSAJES") $("#busquedasMensajes").click();
        if (tipo == "ARCHIVOS") $("#busquedasArchivos").click();
        if (tipo == "AVANCES") $("#busquedasAvances").click();
        return true;
    }
}