var app = new Vue({
    ...
    methods: {
        ...
        editarObjetivo: function (
            ...
            this.limpiarFormularios();
            this.deshabilitarPorDesbloqueo = deshabilitar == '0' || deshabilitar == '' ? false : true;
        ...
        async editar(idCompromiso) {
            ...
            await axios.get(urlKeeps).then(response => {
                ...
                this.formCrear.fecha_inicio_compromiso = this.objEditar.fecha_inicio;
                this.formCrear.fecha_fin_compromiso = this.objEditar.fecha_fin;
                this.formCrear.nombre_compromiso = this.objEditar.nombre_compromiso;
                this.formCrear.detalle_compromiso = this.objEditar.detalle_compromiso != null ? this.objEditar.detalle_compromiso : '';
                this.formCrear.avance_compromiso = this.objEditar.avance_compromiso;
                this.formCrear.avance_id = this.objEditar.avance_id;
                this.formCrear.notas_compromiso = this.objEditar.notas_compromiso != null ? this.objEditar.notas_compromiso : '';
                this.formCrear.cumplimiento = this.objEditar.cumplimiento != null ? this.objEditar.cumplimiento : '';
                this.formCrear.avance = this.objEditar.avance != null ? this.objEditar.avance : '0';
                this.formCrear.tipo_compromiso_id = this.objEditar.tipo_compromiso_id;
                ...
                if (corresp_id.length > 0)
                    getCargaDatosInstitucionCorresponsables("institucion", idCompromiso)
                ...
                this.formCrear.avance = parseFloat(this.formCrear.avance).toFixed(2).toString() + '%';
            }).catch(error => {
                this.cargando = false;
        ...
        async desbloquearObjetivo(id) {
            ...
            axios.post(url, fill).then(response => {
                if (response.data.status == 200) {
                    $(".confirm").click();
                    datatableCargarObjetivos();
                    alertToastSuccess("Registro : " + id + " desbloqueado exitosamente", 3500);
                }
        ...
        async aprobarAvances(id, descripcion) {
            ...
            let result = await swal({
                ...
                if (isConfirm) {
                    ...
                    axios.post(url, fill).then(response => {
                        $(".confirm").click()
                        app.formCrear.avance_compromiso = response.data.descripcion;
                        datatableCargarHistorico();
                        datatableCargarAvances();
                        alertToastSuccess("Registro : " + id + " aprobado exitosamente", 3500)
                    }).catch(error => {
            ...
            if (error.length > 0) {
                ...
                let result = await swal({
                    ...
                    if (isConfirm) {
                        ...
                        data.append('fecha_inicio', app.formCrear.fecha_inicio_compromiso);
                        data.append('fecha_fin', app.formCrear.fecha_fin_compromiso);
                        data.append('nombre_compromiso', app.formCrear.nombre_compromiso);
                        data.append('detalle_compromiso', app.formCrear.detalle_compromiso != null ? app.formCrear.detalle_compromiso : ' ');
                        data.append('avance_compromiso', app.formCrear.avance_compromiso != null ? app.formCrear.avance_compromiso : ' ');
                        data.append('avance_id', app.formCrear.avance_id);
                        data.append('notas_compromiso', app.formCrear.notas_compromiso != null ? app.formCrear.notas_compromiso : ' ');
                        data.append('cumplimiento', app.formCrear.cumplimiento != null ? app.formCrear.cumplimiento : 0);
                        data.append('avance', app.formCrear.avance != null ? app.formCrear.avance : 0);
                        data.append('tipo_compromiso_id', app.formCrear.tipo_compromiso_id);
                        data.append('origen_id', app.formCrear.origen_id);
                        data.append('monitor_id', app.formCrear.monitor_id == null ? 0 : app.formCrear.monitor_id);
                        data.append('estado_porcentaje_id', app.formCrear.estado_porcentaje_id);
                        data.append('estado_id', app.formCrear.estado_id);
                        data.append('institucion_id', app.formCrear.institucion_id);
                        data.append('instituciones_corresponsables', app.formCrear.instituciones_corresponsables);

                        data.append('modifica_corresponsable', app.modifica_corresponsable);
                        data.append('modifica_responsable', app.modifica_responsable);
                        ...
                        axios.post(url, data).then(response => {
                            ...
                            app.registrados = app.dataCargar.registrados;
                            ...
                            app.planificacion = app.dataCargar.planificacion;
        ...
        verificarCambiosResponsables: function () {
            var errores = "";
            if (this.formCrear.tipo_compromiso_id == '' ||
            ...
            if (_nombre_compromiso == '' || _nombre_compromiso == null || _nombre_compromiso.length < 6) {
                errores += "\nDebe llenar el nombre del compromiso mayor a 5 caracteres para guardar";
            }
            if (this.formCrear.detalle_compromiso == '' ||
                this.formCrear.detalle_compromiso == null) {
                errores += "\nDebe llenar el detalle del compromiso";
            }
            var __instituciones_corresponsables = this.formCrear.instituciones_corresponsables;
            if (errores == "") {
                if (this.objEditar.id != 0) {
                    ...
                    if (__instituciones_corresponsables == '')
                        ...
                        $.each(corresp_id, function (key, value) {
                            arregloCorresponsable.push(value.institucion_corresponsable_id);
                        });
                        if (!Array.isArray(__instituciones_corresponsables) && corresp_id.length == 1) {
                            if (arregloCorresponsable.indexOf(__instituciones_corresponsables) == -1)
                                this.modifica_corresponsable = false;
                        } else {
                            ...
                            encontradoCorresponsables = arrayEquals(arregloCorresponsable, __instituciones_corresponsables);
                        }
        ...
        limpiarForm: function () {
            ...
            this.formCrear.id = '0';
            this.formCrear.fecha_inicio_compromiso = new Date().toISOString().slice(0, 10);
            this.formCrear.fecha_fin_compromiso = new Date().toISOString().slice(0, 10);
        ...
        calcular: function () {
            ...
            if (this.formCronograma.numero == '1') {
                ...
                this.formCronograma.meta_acumulada = (parseFloat(this.formCronograma.valor_anterior_meta_acumulada) + parseFloat(this.formCronograma.meta_periodo)).toFixed(2);
                this.formCronograma.cumplimiento_acumulado = (parseFloat(this.formCronograma.valor_anterior_cumplimiento_acumulado) + parseFloat(this.formCronograma.cumplimiento_periodo)).toFixed(2);
            }
            this.formCronograma.pendiente_periodo = (parseFloat(this.formCronograma.meta_periodo) - parseFloat(this.formCronograma.cumplimiento_periodo)).toFixed(2);
            this.formCronograma.pendiente_acumulado = (parseFloat(this.formCronograma.meta_acumulada) - parseFloat(this.formCronograma.cumplimiento_acumulado)).toFixed(2);
        },
        async getKeeps() {
            var urlKeeps = 'consultaEstados';
            let fill = {
                'asignaciones': this.asignaciones,
                'pendientes': this.btnPendientes,
                'temporales': this.btnTemporal,
            }
        ...
        getBusquedas: function () {
            ...
            let gabinete_id_busqueda = $("#gabinete_id_busqueda").val() == "" || $("#gabinete_id_busqueda").val() == null ? 0 : $("#gabinete_id_busqueda").val();
            let institucion_id_busqueda = $("#institucion_id_busqueda").val() == "" || $("#institucion_id_busqueda").val() == null ? 0 : $("#institucion_id_busqueda").val();
            let monitor_busqueda = $("#monitor_busqueda").val() == "" || $("#monitor_busqueda").val() == null ? 0 : $("#monitor_busqueda").val();
        ...
        async guardarPeriodo() {
            ...
            await axios.post(urlKeeps, this.formCronograma).then(response => {
                ...
                if (response.data.status == 200) {
                    ...
                    this.formCrear.cumplimiento = response.data.cumplimiento;
                } else {
                    swal("Cancelado!", "Error al grabar...", "error");
                }
        ...
        async institucionBusqueda() {
            if (this.gabinete_id_busqueda != 0 && this.institucion_anterior == null)
                this.arrayInstitucionBusqueda = [];
            this.cargando = true;
            ...
            iniciar_modal_espera();
            await axios.post(urlKeeps, fill).then(response => {
                this.arrayInstitucionBusqueda = response.data.datos != null ? response.data.datos : [];
                if (this.institucion_anterior != null) {
                    $("#institucion_id_busqueda").val(this.institucion_anterior).change();
                }
                this.cargando = false;
                parar_modal_espera();
            }).catch(error => {
        ...
        async gabineteBusqueda(limpiar = null) {
            ...
            iniciar_modal_espera();
            await axios.post(urlKeeps, fill).then(response => {
                if (this.institucion_id_busqueda == 0 || this.institucion_id_busqueda == "" || this.institucion_id_busqueda == null)
                    this.arrayGabineteBusqueda = response.data.datos != null ? response.data.datos : [];
                else {
                    $("#gabinete_id_busqueda").val(response.data.datos.id).change();
                }
                this.cargando = false;
                parar_modal_espera();
            }).catch(error => {
            ...
            let result = await swal({
                ...
                if (isConfirm) {
                    ...
                    axios.post(url, fill).then(response => {
                        this.cerrarRechazo();
                        datatableCargarObjetivos();
                        alertToastSuccess("Rechazo del objetivo!", 3500);
                    }).catch(error => {
                        $(".confirm").click()
                        swal("Cancelado!", "Error al aprobar...", "error");
                    });
