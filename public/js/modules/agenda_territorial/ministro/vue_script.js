        ...
        methods: {
            ...
            async editarUsuario(id) {
                ...
                await axios.post(urlKeeps,fill).then(response=>{
                    ...
                    this.cargando=false;
                    if(response.data.status==200){
                        this.formCrear.id=response.data.datos.id;
                        this.formCrear.identificacion=response.data.datos.identificacion;
                        this.formCrear.nombres=response.data.datos.nombres;
                        this.formCrear.email=response.data.datos.email;
                        this.formCrear.cargo=response.data.datos.cargo;
                        this.formCrear.extension=response.data.datos.extension;
                     //   this.formCrear.password=response.data.datos.password;
            ...
            async cambiarEstadoUsuario(id,estado){//eliminar un registro 
                let result = await swal(
                    {
                    title: "Estás seguro de realizar esta accion",
                    text: "Al confirmar se "+estado+" el usuario",
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonColor: "#DD6B55",
                    confirmButtonText: "Si!",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        ...
                        .then(response=>{
                            app.cargando=false; //desaparece indicador de cargando
                            datatableCargar();
                            alertToastSuccess("Usuario actualizado exitosamente",3500);
                        })
                        .catch(error => {
                            app.cargando=false; //desaparece indicador de cargando
                            swal("Cancelado!", "Error al grabar...", "error");
