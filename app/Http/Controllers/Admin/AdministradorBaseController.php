...
class AdministradorBaseController extends Controller
{
    ...
    {
        $name = 'AsesorCC';
        $objRole = DB::connection('pgsql_presidencia')->table('core.roles')->where(['name' => $name])->first();
        $objSelect = new SelectController();
    ...
    public function getDatatableLiberacionAsesor($identificacion)
    {
        ...
        $datatable1 = DB::connection('pgsql_presidencia_solicitudes')
    ...
    public function activarAsesor(request $request)
    {
        ...
        try {
            ...
            $resultA = DB::connection('pgsql_presidencia_solicitudes')->table('sessionaudita')->where('id', $request->id)
