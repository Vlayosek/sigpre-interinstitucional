...
*/
  ...
  public function exportarExcelCumplidos(request $request)
  {
    ...
    foreach ($compromisos as $key => $compromiso) {
      ...
      $sheet->setCellValue('K' . $fila_, $compromiso['fecha_revisa']);
      $sheet->setCellValue('L' . $fila_, $compromiso['fecha_cumplido']);
      $sheet->setCellValue('M' . $fila_, $compromiso['fecha_reporte']);
      $sheet->getStyle('A' . $fila_ . ':M' . $fila_)->applyFromArray($this->styleCompromisos);
    }
...
*/
  ...
  private function retornaSelectCompromisoUbicacion()
  {
  ...
  private function retornaAgrupacion($query)
  {
  ...
  protected function cargarDatosDashboard($request, $tipo_detalle, $compromisos = false, $data_post = false)
  {
    ...
    try {
      ...
      if ($objRepositorio->validarArregloVacio($provincia_filtro)) $data = $data->whereIn('provincia_.id', $provincia_filtro);;
      ...
      $data = $data->where('provincia_.nivel', 3)
        //->where('provincia_.id', '<>', 1074)
        ->where('provincia_.descripcion', '<>', 'EXTERIOR');
      $data = $data->whereBetween('compromisos.fecha_inicio', [$request->fecha_inicio, $request->fecha_fin]);
  ...
  protected function consultarArregloDatosCompromisos($compromiso)
  {
    ...
    $data[7] = $compromiso['fecha_reporte_'];
